# -*- coding: utf-8 -*-
"""
Created: Thur 21 August 2024
Description: Scripts to calibrate parameters used in logistic funcation of energy data
...
variable = "oil_demand_local"  # need to hard coding
...
def logistic_functions(
    ...
    # exp_term = np.exp(-k * (x / ref_x - x0))
    exp_term = np.exp(-k) / (x**x0)
    log_term = L0 + L / (1 + exp_term)
    if variable == "oil_demand_local":
        x_pop = np.array([cal_data_dep_pop[i] for i in range(len(x))])
        ref = ref * x_pop / 1000000
    return log_term * ref
...
def parameter_calibrating(
    ...
    **kwargs
    ...
    global cal_data_dep
    cal_data_dep = cal_data_input.iloc[-len(cal_data_dep_all), 1:].values.tolist()
    if variable == "oil_demand_local":
        global cal_data_dep_pop
        cal_data_dep_pop = cal_data_input.iloc[-1, 1:].values.tolist()
    ...
    for i in range(cal_data_num):
        ...
        cal_results.loc[cal_data["parameter"][i], ["L0", "L", "k", "x0", "ref"]] = n_par
...
cal_data_dep_ref = {}
for pos in range(len(cal_data_dep_all)):
    cal_data_dep_ref[cal_data_dep_all[pos]] = input_data_info["dependency_variable"][
        pos
    ]["dep_ref"]
...
for region in regions:
    ...
    cal_data_input = cal_data_input.dropna(axis=1, how="any")
