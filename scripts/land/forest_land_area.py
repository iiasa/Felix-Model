...
Description: Scripts to aggregate forest land area from FAOSTAT to FeliX regions
...
raw_forest_land = pd.DataFrame()
for raw_data_file in raw_data_info["data_file"]:
    logging.info(f"Read raw data of {raw_data_file}")
    raw_forest_land_data = pd.read_csv(
        path_data_raw / raw_data_file,
        encoding="latin1",
    ).rename(columns={"Area": "country"})

    raw_forest_land = pd.concat(
        [raw_forest_land, raw_forest_land_data],
        ignore_index=True,
    )
    del raw_forest_land_data
...
def data_restructure(
    ...
    **kwargs,
    ...
    '''
        ...
        Forest Land[Africa],x,x,x,...
        Forest Land[AsiaPacific],x,x,x,...
        ...
        Forest Land[WestEu],x,x,x,...
    '''
    ...
    **kwargs
    ...
    for region in regions:
        entry = {
            "parameter (unit: ha)": f"Forest Land[{region}]",
        }
...
cleaned_land = data_cleaning(raw_forest_land, concordance_table)
