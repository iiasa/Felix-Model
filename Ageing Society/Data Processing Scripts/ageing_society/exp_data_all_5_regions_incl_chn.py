"""
Created: Mon 10 Mar 2025
Description: Scripts to convert national expenditure data into five FeliX regions. China must be included.
        ...
        for year in years:
            ...
            if len(np.unique(cleaned_exp_region_year["country"])) > 1:
                ...
                for felix_region in np.unique(cleaned_exp_pop_merge["felix_region"]):
                    ...
                    if felix_region == "AsiaPacific":
                        if "china" in list(cleaned_exp_pop_region["country"]):
                            entry = {
                            ...
                            for variable in ["household_size", "edu_level", "value"]:
                                entry[variable] = sum(
                                    ...
                                    * np.array(
                                        cleaned_exp_pop_region["value_population"]
                                    )
...
file_name = f"personal_data_in_felix_classification_time_series_incl_china.csv"
