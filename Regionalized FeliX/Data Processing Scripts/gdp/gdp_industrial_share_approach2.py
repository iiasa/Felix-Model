# -*- coding: utf-8 -*-
"""
Created: Fri 27 Septem 2024
Description: Scripts to calculated shares of industrial value added in GDP for 5 FeliX regions
    ...
    for raw_data_file in raw_data_files:
        ...
        if download_method == "api":
            ...
            with open(path_raw_data_folder / raw_data_file) as fact_file:
                raw_ind_share_ = json.load(fact_file)
            for data_point in raw_ind_share_:
                if data_point["value"] == "":
                    data_point["value"] = np.nan
                    del data_point

            raw_ind_share_ = pd.DataFrame(raw_ind_share_)
            raw_ind_share = pd.concat(
                [raw_ind_share, raw_ind_share_],
                ignore_index=True,
            )
            del raw_ind_share_
        else:
...
if raw_data_dependency:
    raw_ind_share_dep = pd.DataFrame()
    if raw_data_info["datasource"] == "wittgenstein":
        ...
        for raw_data_file_dep in raw_data_files_dep:
            ...
            if download_method == "api":
                ...
                with open(
                    ...
                    raw_ind_share_dep_ = json.load(fact_file_dep)

                for data_point_dep in raw_ind_share_dep_:
                    if data_point_dep["value"] == "":
                        data_point_dep["value"] = np.nan
                        del data_point_dep

                raw_ind_share_dep_ = pd.DataFrame(raw_ind_share_dep_)
                raw_ind_share_dep = pd.concat(
                    [raw_ind_share_dep, raw_ind_share_dep_],
                    ignore_index=True,
                )
                del raw_ind_share_dep_
            else:
                logging.warning("The download method is not API")
                raise KeyError
    elif raw_data_info["datasource"] == "unpd":
    ...
    **kwargs
    ...
    for year in years:
        for region in regions:
            ...
            va_share = (
                raw_data_region_year_merge_nonana["value"].sum()
            ...
            entry = {
                ...
                "va_ind": raw_data_region_year_merge["value_dep"].sum() * va_share,
                "value": va_share,
            }
...
def data_restructure(
    ...
    **kwargs,
    ...
    for region in regions:
        entry = {
            f"parameter": f"Share of VA Industry[{region}]",
        }
        entry_gdp = {
            f"parameter": f"GDP in 2015 $[{region}]",
        }
        entry_va_ind = {
            f"parameter": f"VA of Industry in 2015 $[{region}]",
        }
        for year in range(1900, 2101):
            if year in year_avail:
                ...
                entry_va_ind[year] = data_by_all["va_ind"].values[0]
            else:
                entry[year] = ""
                entry_gdp[year] = ""
                entry_va_ind[year] = ""

        restructured_data.extend([entry, entry_gdp, entry_va_ind])
        del entry, entry_gdp, entry_va_ind
...
cleaned_ind_share = data_cleaning(
    raw_data=raw_ind_share,
    raw_data_source=data_source,
    concordance=concordance_table,
    raw_data_dep=raw_ind_share_dep,
)
...
restructured_gdp = data_restructure(
    cleaned_data=cleaned_ind_share,
)
