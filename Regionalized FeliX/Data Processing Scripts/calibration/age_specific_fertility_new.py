...
variable = "age_specific_fertility_new"  # need to hard coding
...
def logistic_functions(
    ...
    """
    if variable == "age_specific_fertility_new":
        x_tfr = np.array([cal_data_dep_tfr[int(i)] for i in x.tolist()])
        imp_tfr = x_tfr**p_tfr

        x_mys = np.array([cal_data_dep_mys[int(i)] for i in x.tolist()])
        imp_mys = x_mys**p_mys

        return L0 + L / (1 + np.exp(-k) / (imp_tfr * imp_mys))
    else:
        logging.warning(f"Please double check the logistic function of {variable}")
        raise KeyError
...
def parameter_calibrating(
    ...
    **kwargs
    ...
    if len(cal_data_dep_all) == 1:
        cal_data_dep = cal_data_input.iloc[-1, 1:].values.tolist()
    else:
        if variable == "age_specific_fertility_new":
            logging.info(f"Start to predefine the dependent variables")
            global cal_data_dep_tfr
            cal_data_dep_tfr = cal_data_input.iloc[
                -len(cal_data_dep_all), 1:
            ].values.tolist()
            ...
            logging.error(f"Need to predefine the dependent variables")
            raise KeyError
