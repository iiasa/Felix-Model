...
logging.info("Filling missing data with alternative data")
for pos in range(len(raw_comtrade)):
    if (
        pd.isna(raw_comtrade.loc[pos, "qtyUnitAbbr"])
        and float(raw_comtrade.loc[pos, "qty"]) == 0
    ):
        raw_comtrade.loc[pos, "qtyUnitAbbr"] = raw_comtrade.loc[pos, "altQtyUnitAbbr"]
        raw_comtrade.loc[pos, "qty"] = raw_comtrade.loc[pos, "altQty"]
...
for unit in units:
    ...
    if unit == "1000 KG":
        raw_comtrade_unit["qty"] = raw_comtrade_unit["qty"] * 1000  # convert to kg
    elif unit == "m³":
        raw_comtrade_unit["qty"] = raw_comtrade_unit["qty"] * 2.2  # convert to kg
    elif unit == "1000 m³":
        raw_comtrade_unit["qty"] = raw_comtrade_unit["qty"] * 2200  # convert to kg
    elif unit == "TJ":
        raw_comtrade_unit["qty"] = raw_comtrade_unit["qty"] * 21692  # convert to kg
...
def data_restructure(
    ...
    **kwargs,
    ...
    To restructure data into the format:
      '''
          Time,1950,1951,1952,...
          Gas Comtrade[Africa, Africa, Africa],x,x,x,...
          Gas Comtrade[Africa, Africa,AsiaPacific],x,x,x,...
          ......
          Gas Comtrade[WestEu, WestEU,WestEu],x,x,x,...
          Gas Comtrade[WestEu, WestEu,World],x,x,x,...
      '''
    ...
    for reporter in regions:
        for partner in regions:
            # for partner2 in regions:
            entry = {
                f"parameter": f"Gas Comtrade {unit}[{reporter},{partner},World]",
            }
