# -*- coding: utf-8 -*-
"""
Created: Sun 19 October 2025
Description: Scripts to aggregate egg production FAOSTAT to FeliX regions
...
raw_egg_production = pd.DataFrame()
for raw_data_file in raw_data_info["data_file"]:
    logging.info(f"Read raw data of {raw_data_file}")
    raw_egg_production_ = pd.read_csv(
        path_data_raw / raw_data_file,
        encoding="latin1",
    ).rename(columns={"Area": "country"})

    raw_egg_production = pd.concat(
        [raw_egg_production, raw_egg_production_],
        ignore_index=True,
    )

    del raw_egg_production_
...
def data_cleaning(
    ...
    **kwargs
    ...
    cleaned_egg_production = []
    for region in regions:
        num_country_ = num_country[region]
        for year in years:
            try:
                raw_data_region_year_ = raw_data_merge_groups.get_group((region, year))
            except KeyError:
                continue

            if len(np.unique(raw_data_region_year_["country"])) / num_country_ > 0.85:
                ...
                cleaned_egg_production.append(entry)
                del entry, raw_data_region_year_

    cleaned_egg_production = pd.DataFrame(cleaned_egg_production)

    return cleaned_egg_production
...
def data_restructure(
    ...
    **kwargs,
    ...
    '''
        ...
        Production rate of animal food[Africa,Eggs],x,x,x,...
        Production rate of animal food[AsiaPacific,Eggs],x,x,x,...
        ...
        Production rate of animal food[WestEu,Eggs],x,x,x,...
    '''
    ...
    **kwargs
    ...
    for region in regions:
        entry = {
            "parameter (in tonnes)": f"Production rate of animal food[{region},Eggs]",
        }
        for year in range(1900, 2101):
            if year in years:
                try:
                    cleaned_egg_production_ = clean_data_groups.get_group(
                        (region, year)
                    )

                    entry[year] = cleaned_egg_production_["value"].values[0]
                except KeyError:
                    entry[year] = np.nan
            else:
                entry[year] = np.nan
...
cleaned_egg_production = data_cleaning(raw_egg_production, concordance_table)

logging.info(f"Start restructuring the cleaned data")
restructured_egg_production = data_restructure(cleaned_egg_production)
logging.info("Finish data cleaning")

logging.info("Write clean data into a .csv file")
restructured_egg_production.to_csv(
