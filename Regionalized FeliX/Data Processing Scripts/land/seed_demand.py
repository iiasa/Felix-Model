...
Description: Scripts to calculate seed demand from FAOSTAT to FeliX regions
...
def data_cleaning(
    ...
    **kwargs
    ...
    for region in regions:
        ...
        for year in years:
            # raw_data_seed_amount = [np.nan] * len(food_categories)
            for food_category in food_categories:
                try:
                    raw_data_seed_ = raw_data_merge_groups.get_group(
                ...
                if len(np.unique(raw_data_seed_["country"])) / num_country_ > 0.75:
                    entry = {
                        ...
                        "value": raw_data_seed_["value"].sum() * 1000,
                        "unit": "tonnes",
                    }

                    cleaned_seed_data.append(entry)
                    del entry, raw_data_seed_, food_category
            del year
        del region

    cleaned_seed_data = pd.DataFrame(cleaned_seed_data)

    return cleaned_seed_data
...
def data_restructure(
    ...
    **kwargs,
    ...
    '''
        ...
        Seed demand in tonnes[Africa,Pulses],x,x,x,...
        Seed demand in tonnes[AsiaPacific,Pulses],x,x,x,...
        ...
        Seed demand in tonnes[WestEu,OtherCrops],x,x,x,...
    '''
    ...
    **kwargs
    ...
    for region in regions:
        for food_category in food_categories:
            entry = {
                "parameter": f"Seed demand in tonnes[{region},{food_category}]",
            }
            for year in range(1900, 2101):
                if year in years:
                    try:
                        cleaned_seed_data_ = clean_data_groups.get_group(
                            (region, year, food_category)
                        )
                        entry[year] = cleaned_seed_data_["value"].values[0]
                    except KeyError:
                        entry[year] = np.nan
                else:
                    entry[year] = np.nan
...
cleaned_seed_data = data_cleaning(raw_seed_data, concordance_table)

logging.info(f"Start restructuring the cleaned data")
restructured_seed_data = data_restructure(cleaned_seed_data)
logging.info("Finish data cleaning")

logging.info("Write clean data into a .csv file")
restructured_seed_data.to_csv(
