...
Description: Scripts to calculate processed demand from FAOSTAT to FeliX regions
...
data_variable = "processed_demand"
data_source = "faostat"
...
raw_processed_data = pd.DataFrame()
for raw_data_file in raw_data_info["data_file"]:
    logging.info(f"Read raw data of {raw_data_file}")
    raw_processed_data_ = pd.read_csv(
        path_data_raw / raw_data_file,
        encoding="latin1",
    ).rename(columns={"Area": "country"})

    raw_processed_data = pd.concat(
        [raw_processed_data, raw_processed_data_],
        ignore_index=True,
    )

    del raw_processed_data_
...
def data_cleaning(
    ...
    **kwargs
    ...
    for region in regions:
        ...
        for year in years:
            # raw_data_processed_amount = [np.nan] * len(food_categories)
            for food_category in food_categories:
                try:
                    raw_data_processed_ = raw_data_merge_groups.get_group(
                        (region, year, food_category)
                    )
                except KeyError:
                    continue

                if len(np.unique(raw_data_processed_["country"])) / num_country_ > 0.75:
                    entry = {
                        "region": region,
                        "year": year,
                        "food_category": food_category,
                        "value": raw_data_processed_["value"].sum() * 1000,
                        "unit": "tonnes",
                    }

                    cleaned_processed_data.append(entry)
                    del entry, raw_data_processed_, food_category
            del year
        del region

    cleaned_processed_data = pd.DataFrame(cleaned_processed_data)
...
def data_restructure(
    ...
    **kwargs,
    ...
    '''
        ...
        Processed demand in tonnes[Africa,Pulses],x,x,x,...
        Processed demand in tonnes[AsiaPacific,Pulses],x,x,x,...
        ...
        Processed demand in tonnes[WestEu,OtherCrops],x,x,x,...
    '''
    ...
    **kwargs
    ...
    for region in regions:
        for food_category in food_categories:
            entry = {
                "parameter": f"Processed demand in tonnes[{region},{food_category}]",
            }
            for year in range(1900, 2101):
                if year in years:
                    try:
                        cleaned_processed_data_ = clean_data_groups.get_group(
                            (region, year, food_category)
                        )
                        entry[year] = cleaned_processed_data_["value"].values[0]
                    except KeyError:
                        entry[year] = np.nan
                else:
                    entry[year] = np.nan
...
cleaned_processed_data = data_cleaning(raw_processed_data, concordance_table)

logging.info(f"Start restructuring the cleaned data")
restructured_processed_data = data_restructure(cleaned_processed_data)
logging.info("Finish data cleaning")

logging.info("Write clean data into a .csv file")
restructured_processed_data.to_csv(
