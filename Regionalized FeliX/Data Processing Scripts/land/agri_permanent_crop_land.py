...
Description: Scripts to aggregate agricultural permanent crop land area from FAOSTAT to FeliX regions
...
data_variable = "permanent_crop_land"
data_source = "faostat"
...
raw_permanent_crop_land = pd.DataFrame()
for raw_data_file in raw_data_info["data_file"]:
    logging.info(f"Read raw data of {raw_data_file}")
    raw_permanent_crop_land_data = pd.read_csv(
        path_data_raw / raw_data_file,
        encoding="latin1",
    ).rename(columns={"Area": "country"})

    raw_permanent_crop_land = pd.concat(
        [raw_permanent_crop_land, raw_permanent_crop_land_data],
        ignore_index=True,
    )
    del raw_permanent_crop_land_data
...
def data_restructure(
    ...
    **kwargs,
    ...
    '''
        ...
        Permanent crops[Africa],x,x,x,...
        Permanent crops[AsiaPacific],x,x,x,...
        ...
        Permanent crops[WestEu],x,x,x,...
    '''
    ...
    **kwargs
    ...
    for region in regions:
        entry = {
            "parameter (unit: ha)": f"Permanent crops[{region}]",
        }
...
cleaned_land = data_cleaning(raw_permanent_crop_land, concordance_table)
