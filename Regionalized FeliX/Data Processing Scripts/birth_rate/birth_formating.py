...
Description: Scripts to process total birth rate data by region from UNPD to 5 FeliX regions
...
# predefine the data source of birth_rate
# data sources are wittgensteinwittgenstein, world_bank, unpd
variable = "total_birth"
data_source = "unpd"
...
if raw_data_info["datasource"] == "wittgenstein":
    ...
    for raw_data_file in raw_data_files:
        ...
        with open(path_raw_data_folder / raw_data_file) as fact_file:
            raw_birth_rate_ = json.load(fact_file)
        raw_birth_rate_ = pd.DataFrame(raw_birth_rate_)

        raw_birth_rate_ = raw_birth_rate_.loc[
            raw_birth_rate_["variant"] == "Median"
        ].reset_index(drop=True)
        raw_birth_rate = pd.concat(
            [raw_birth_rate, raw_birth_rate_],
            ignore_index=True,
        )
        del raw_birth_rate_
...
def data_restructure(
    ...
    **kwargs,
    ...
    To restructure data into the format:
      '''
          Time,1950,1951,1952,...
          Total Birth Rate[Africa],x,x,x,...
          Total Birth Rate[AsiaPacific],x,x,x,...
          ......
          Total Birth Rate[WestEU],x,x,x,...
      '''
    ...
    for region in regions:
        entry = {"parameter": f"Total Birth Rate[{region}]"}
...
cleaned_birth_rate = data_cleaning(
    raw_data=raw_birth_rate,
...
restructured_birth_rate = data_restructure(cleaned_data=cleaned_birth_rate)
logging.info(f"Finish restructuring the cleaned data")

logging.info(f"Start writing the restructured data")
restructured_birth_rate.to_csv(
    path_clean_data_folder.joinpath(f"birth_rate_by_time_series_{data_source}.csv"),
    encoding="utf-8",
    index=False,
)
