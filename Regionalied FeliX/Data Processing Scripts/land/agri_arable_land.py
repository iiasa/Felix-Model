...
Description: Scripts to aggregate agricultural arable land area from FAOSTAT to FeliX regions
...
data_variable = "arable_land_area"
data_source = "faostat"
...
raw_arable_land = pd.DataFrame()
for raw_data_file in raw_data_info["data_file"]:
    logging.info(f"Read raw data of {raw_data_file}")
    raw_arable_land_data = pd.read_csv(
        path_data_raw / raw_data_file,
        encoding="latin1",
    ).rename(columns={"Area": "country"})

    raw_arable_land = pd.concat(
        [raw_arable_land, raw_arable_land_data],
        ignore_index=True,
    )
    del raw_arable_land_data
...
def data_restructure(
    ...
    **kwargs,
    ...
    '''
        ...
        Arable Land[Africa],x,x,x,...
        Arable Land[AsiaPacific],x,x,x,...
        ...
        Arable Land[WestEu],x,x,x,...
    '''
    ...
    **kwargs
    ...
    for region in regions:
        entry = {
            "parameter (unit: ha)": f"Arable Land[{region}]",
        }
...
cleaned_land = data_cleaning(raw_arable_land, concordance_table)
