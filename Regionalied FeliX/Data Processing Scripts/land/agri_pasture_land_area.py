...
Description: Scripts to aggregate pasture land area from FAOSTAT to FeliX regions
...
data_variable = "pasture_land_area"
data_source = "faostat"
...
raw_pasture_land = pd.DataFrame()
for raw_data_file in raw_data_info["data_file"]:
    logging.info(f"Read raw data of {raw_data_file}")
    raw_pasture_land_data = pd.read_csv(
        path_data_raw / raw_data_file,
        encoding="latin1",
    ).rename(columns={"Area": "country"})

    raw_pasture_land = pd.concat(
        [raw_pasture_land, raw_pasture_land_data],
        ignore_index=True,
    )
    del raw_pasture_land_data
...
def data_restructure(
    ...
    **kwargs,
    ...
    '''
        ...
        Pasture Land Area[Africa],x,x,x,...
        Pasture Land Area[AsiaPacific],x,x,x,...
        ...
        Pasture Land Area[WestEu],x,x,x,...
    '''
    ...
    **kwargs
    ...
    for region in regions:
        entry = {
            "parameter (unit: ha)": f"Pasture Land Area[{region}]",
        }
...
cleaned_land = data_cleaning(raw_pasture_land, concordance_table)
